...
const TRANSLATIONS = {
  ...
  ja: {
    ...
    voiceStatusTranscribing: '✍️ 文字にしています…',
    ...
    voiceNoteWebspeechUnsupported: '⚠️ この端末では「標準」が使えないため「高精度（Whisper）」をおすすめします。',
    voiceNoteRecordingUnsupported: '⚠️ この端末では録音が使えません。「標準」をためしてね。',
    voiceNoteNoKey: '⚠️ OpenAIのAPIキーが未設定のため「高精度」は使えません。「標準」をためしてね。',
    ...
    scoreHowtoList: '<li>🚶 たくさん歩く</li><li>📍 いろんなスポットに行く</li><li>📷 写真をたくさん撮る</li><li>🏷️ 写真にスポットのタグを付ける</li><li>📝 感想や写真コメントを書く</li><li>⏱️ 1時間以内におわらせる</li><li>🎯 Googleの所要時間に近いペースで歩く</li><li>🎁 ARカメラでキャラをつかまえる（レアはボーナス！）</li>',
  ...
  en: {
    ...
    genReport: '⚠️ Report this character',
    ...
    voiceStatusTranscribing: '✍️ Transcribing…',
    ...
    voiceNoteWebspeechUnsupported: '⚠️ "Standard" is not available on this device — try "High accuracy (Whisper)".',
    voiceNoteRecordingUnsupported: '⚠️ Recording is not available on this device — try "Standard".',
    voiceNoteNoKey: '⚠️ OpenAI API key is not set, so "High accuracy" is unavailable — try "Standard".',
    ...
    scoreHowtoList: '<li>🚶 Walk a lot</li><li>📍 Visit many spots</li><li>📷 Take many photos</li><li>🏷️ Tag photos with the spot they belong to</li><li>📝 Write impressions and photo comments</li><li>⏱️ Finish within 60 minutes</li><li>🎯 Pace close to Google\'s estimated time</li><li>🎁 Catch characters with the AR camera (rare = bonus!)</li>',
  ...
  elementary: {
    ...
    voiceStatusTranscribing: '✍️ 文字（もじ）にしています…',
    ...
    voiceNoteWebspeechUnsupported: '⚠️ このたんまつでは「標準（ひょうじゅん）」が使（つか）えないので「高精度（こうせいど）（Whisper）」がおすすめだよ。',
    voiceNoteRecordingUnsupported: '⚠️ このたんまつではろくおんが使（つか）えません。「標準（ひょうじゅん）」をためしてね。',
    voiceNoteNoKey: '⚠️ OpenAIのAPIキーがないので「高精度（こうせいど）」は使（つか）えません。「標準（ひょうじゅん）」をためしてね。',
    ...
    scoreHowtoList: '<li>🚶 たくさん歩（ある）く</li><li>📍 いろんなスポットに行（い）く</li><li>📷 写真（しゃしん）をたくさん撮（と）る</li><li>🏷️ 写真にスポットの名前（なまえ）タグを付（つ）ける</li><li>📝 感想（かんそう）や写真のコメントを書（か）く</li><li>⏱️ 1時間（じかん）いないにおわらせる</li><li>🎯 Googleの時間（じかん）に近（ちか）いペースで歩（ある）く</li><li>🎁 ARカメラでキャラをつかまえる（レアはボーナス！）</li>',
