...
const TRANSLATIONS = {
  ...
  elementary: {
    ...
    btnArCapturedClose: 'やったー！',
    arCaution: '⚠️ まわりに ちゅうい！',
    arSearching: 'キャラをさがそう！\nスポットにちかづいて、カメラをむけてみよう',
    arDistanceFmt: 'あと {m}m',
    arFarFmt: 'まだとおいみたい…あと {m}m ちかづこう！',
    arNear: 'ちかくに いるみたい…！\nカメラをまわりに むけてみよう',
    arNoSensor: 'ちかくに いるよ！（ほういセンサーがつかえないので きょりだけで はんていちゅう）',
    arNoGps: 'GPSがつかえないよ',
    arFoundFmt: '{name} が あらわれた！ シャッターをおして つかまえよう！',
    arCapturedFmt: '{name} を つかまえた！',
    arNoCharToday: 'ここには キャラは いないみたい…\nでもきねん写真（しゃしん）はとれるよ！',
    arCameraError: 'カメラをきどうできませんでした。ブラウザのカメラきょかをかくにんしてね。',
...
export const WIZARD_SPOT_HINTS = {
  ...
  en: {
    historic: [
      ...
      'Snap where you felt the most reverent atmosphere ⛩️',
    ...
    museum: [
      ...
      'Capture exhibits that made an impression (no flash ⚠️)',
  ...
  elementary: {
    historic: [
      ...
      '昔（むかし）の人もここを通（とお）ったかな？当時（とうじ）のようすを想像（そうぞう）しながら📷',
      '屋根（やね）や柱（はしら）の細（こま）かいもようを見（み）つけたら撮（と）ってみて',
      '一番（いちばん）神聖（しんせい）な空気（くうき）を感（かん）じた所（ところ）を写真（しゃしん）に⛩️',
    ...
    sweets: [
      ...
      'お店（みせ）のふんいきを写真（しゃしん）にのこしてね🥐',
      '一番（いちばん）おいしそうなお菓子（かし）をアップで📸',
      'においまで伝（つた）わりそうな一枚（いちまい）を撮（と）ってみよう',
      '季節（きせつ）げんていのお菓子（かし）はあるかな？',
      'お店（みせ）の人に「写真（しゃしん）撮（と）っていい？」と聞（き）いてみよう',
      'お菓子（かし）のならべ方（かた）を撮影（さつえい）',
    ...
    toy: [
      ...
      '入（い）り口（ぐち）や看板（かんばん）の楽（たの）しいかざりを撮影（さつえい）',
    ...
    museum: [
      ...
      '印象（いんしょう）にのこったてんじを撮（と）ろう（フラッシュ禁止（きんし）に注意（ちゅうい）⚠️）',
      '撮影（さつえい）OKの作品（さくひん）なら、近（ちか）づいてアップで📸',
      'パンフレットや看板（かんばん）も思（おも）い出（で）として',
      '自分（じぶん）が一番（いちばん）きょうみを持（も）ったものを撮影（さつえい）',
      '建物（たてもの）の天井（てんじょう）や階段（かいだん）の美（うつく）しさを入（い）れて',
      'てんじの前（まえ）でとまる人の様子（ようす）も',
      'ミュージアムショップのおもしろいお土産（みやげ）を📷',
      'あんないの人に「ここで一番（いちばん）有名（ゆうめい）なのは？」と聞（き）いてみよう',
      'レポートに書（か）きたいてんじを一枚（いちまい）📸',
    ],
    science: [
      '大（おお）きなてんじ物（ぶつ）を全身（ぜんしん）で撮（と）ろう🦖',
      'さわれる実験（じっけん）コーナーを楽（たの）しんで📷',
      '目（め）を引（ひ）いた標本（ひょうほん）や模型（もけい）を📸',
      '入口（いりぐち）のてんじやプラネタリウムを撮影（さつえい）',
      '仕組（しく）みがおもしろいと思（おも）った装置（そうち）を撮（と）って、家（いえ）で説明（せつめい）',
      '大（おお）きさをくらべるために自分（じぶん）も入（はい）った写真（しゃしん）を',
      '説明（せつめい）パネルで「へぇ！」と思（おも）ったものを撮（と）ろう',
      '科学者（かがくしゃ）気分（きぶん）でかんさつしたてんじを一枚（いちまい）',
      '動物（どうぶつ）の標本（ひょうほん）や恐竜（きょうりゅう）の化石（かせき）を📸',
      '実験（じっけん）しているようすを一枚（いちまい）に📸',
    ],
    dagashi: [
      'たないっぱいの駄菓子（だがし）を撮（と）ろう🍬',
      '一番（いちばん）気（き）になったお菓子（かし）をアップで📷',
      'ねだんの付（つ）いたふだもいっしょに撮（と）ると思（おも）い出（で）に',
      '自分（じぶん）がえらんだお菓子（かし）の山（やま）を📸',
      'お店（みせ）のなつかしいふんいきを写真（しゃしん）に',
      'はじめて見（み）る駄菓子（だがし）を発見（はっけん）したら一枚（いちまい）',
      'ガラスびんに入（はい）ったあめやくじを撮（と）ろう',
      'お店（みせ）の看板（かんばん）や入（い）り口（ぐち）を📷',
      'お店（みせ）の人と話（はな）したら、その様子（ようす）も思（おも）い出（で）に（許可（きょか）をとってね）',
      '同（おな）じお菓子（かし）の色（いろ）ちがいをならべて撮影（さつえい）',
    ],
    other: [
      '気（き）になったものを自由（じゆう）に撮（と）ろう📷',
      '何（なに）が一番（いちばん）おもしろいと思（おも）った？',
      '自分（じぶん）らしいこうずで一枚（いちまい）📸',
      '友（とも）だちや家族（かぞく）といっしょに撮（と）るのもいいよ',
      'ここでの思（おも）い出（で）を写真（しゃしん）にのこそう',
      '看板（かんばん）や入（い）り口（ぐち）を撮（と）るのもおすすめ',
      'まわりの人の様子（ようす）も（許可（きょか）をとってね）',
      'かんじたふんいきをあらわす一枚（いちまい）📷',
      'こまかいディテールに注目（ちゅうもく）してアップで',
      '全体（ぜんたい）の様子（ようす）を伝（つた）える広（ひろ）いこうずで',
    ],
  },
};
