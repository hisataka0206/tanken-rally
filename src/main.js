...
async function openChargen() {
  ...
  // 生成経路（実API/モック）はコンソールにのみ出力（画面バッジは廃止）。
...
const TEST_PLAN = [
  ...
  { title: '11. トップ・再開導線（UI改善）', items: [
    ...
    { id: 't-ui-newstart',  do: '「あたらしく はじめる」を押す', check: '駅えらびへ進む（中断は破棄）' },
    { id: 't-ui-noindicator',do: '探検を再開して各画面を見る', check: '黄色い「再開中の冒険」インジケータが出ない' },
  ]},
  { title: '12. テキスト最小化（UI改善）', items: [
    { id: 't-ui-station',   do: '駅えらび画面を見る', check: '路線がカテゴリ化（地下鉄/名鉄/JR）＋短縮名、枠と▼で順序表現、駅チップはルビ表示で括弧書きが無い' },
    { id: 't-ui-spotcard',  do: 'スポット候補のカードを見る', check: '距離・住所が無く、駅からの方位アイコン（矢印）が出る。カテゴリは絵文字のみ' },
    { id: 't-ui-routestats',do: 'ルート画面の統計を見る', check: '「総距離/推定時間/スポット数」ラベルが無く「3.1km ／ 約○分 ／ 3スポット」。区間ヘッダーは括弧なし、SV下に「→次は◯◯方面」が無い' },
    { id: 't-ui-zukan',     do: '図鑑を開く', check: 'アカウント名・ログアウト・ヘルプ「？」が無い。集計が「7 / 90」の数字表記' },
    { id: 't-ui-history',   do: '履歴から「つづき」を押す', check: '即ローディングが出る。内部フォルダ名（ランダムID）や訪問順の羅列が出ない' },
  ]},
  { title: '13. ボタン統一・完了導線（UI改善）', items: [
    { id: 't-ui-btnarrow',  do: '各ボタンの文言を見る', check: '文字と矢印の重複が無い（「次へ →」等）。戻る系は「←」のみ' },
    { id: 't-ui-reportbtn', do: 'ノート画面のボタンを見る', check: 'ツール（メモ整理/なかま発見/PDF）とナビ（←/たんけん完了）が分離、アイコン混在なし、縦に間延びしない' },
    { id: 't-ui-complete',  do: '「たんけん完了！」を押す', check: 'スコアが表示され、閉じるとトップ画面に戻る' },
  ]},
  { title: '14. キャラ生成（UI改善・課題4）', items: [
    { id: 't-ui-3chars',    do: 'なかまを発見して待つ', check: 'シルエットが必ず3体そろう（2体に減らない）' },
    { id: 't-ui-fasttrans', do: '名前を選ぶ', check: 'すぐ完了画面へ切り替わる（保存待ちで固まらない／説明文は後から図鑑に入る）' },
    { id: 't-ui-motif',     do: '生成キャラの見た目を見る', check: 'モチーフと体の形が噛み合っている（魚なら魚の形＋ヒレ等）' },
    { id: 't-ui-wherebonus',do: '「いまどこ」を一度も使わずに生成する', check: 'レア度が1段上がる（減点ではなく加点）' },
  ]},
];
