...
2. DevTools のコンソールにファイルの中身を**そのまま**貼って実行
3. 画面を切り替えたら `tekutanMeasure()` を再実行

出力は6分類。**どれにも入れずに黙って除外しない**こと（分母から隠すと改善が数字に出ない）。

| 分類 | 内容 | 削減目標 |
|---|---|---|
| ui | 指示・ラベル・ボタン | 導線画面は20字以内 |
| guide | 道案内（区間・ターン指示） | 対象（従来は誤って除外していた） |
| story | 読み物（駅の由来・キャラ話） | 対象外（世界観） |
| data | 固有名詞・ユーザーデータ | 対象外（選択肢そのもの） |
| icons | 絵文字・矢印（個数） | 対象外 |
| furigana | ルビ | 別枠 |
