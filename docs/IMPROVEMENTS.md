...
- 参照: CM議論 `docs/cm-howto-storyboard.md`／地図PDF実装 `src/utils/pdf.js`・地図描画 `src/main.js`。
...
- **系譜＝デザインDNAの実装の芯**: 親 Legend の **基本フォルム＋形に出る特徴（[[フォルムファースト設計]]の14フォルム＋特徴軸）を"家族の糸"として固定**し、子は同DNAを保ったまま色・モチーフ・格を振る。＝ポケモンの[[進化系統]]「視覚DNA一貫×エスカレーション」（`docs/pokemon-character-design-analysis.md` §3）の沿線版。`buildPrompt` に「親のフォルム＋特徴を継承」オプションを足す形で接続できる。
- **要検討（後で詰める）**:
